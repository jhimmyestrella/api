...
<div class="row clearfix">
    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
        <div class="card">
            ...
            <div class="body">
                <form action="{{ route('user.update', $user->id) }}" method="POST" id="form_advanced_validation" enctype="multipart/form-data">
                    @method('PUT')
                    @csrf            
                    <div class="col-sm-12">
                        <div class="col-sm-3">
                            <div class="form-group form-float">
                                <div class="form-line {{ $errors->has('name') ? 'focused error' : '' }}">
                                    <label>Nombre*</label>
                                    <input type="text" name="name" class="form-control" required value="{{  $errors->has('name') ? old('name') : $user->name   }} " maxlength="255" >                        
                                </div>
                                @if ($errors->has('name'))
                        ...
                        <div class="col-sm-3">
                            <div class="form-group form-float">
                                <div class="form-line {{ $errors->has('lastName') ? 'focused error' : '' }}">
                                    ...
                                    <input type="text" name="lastName" class="form-control" required value="{{  $errors->has('lastName') ? old('lastName') : $user->lastName   }} " maxlength="255" >                        
                                </div>
                        ...
                        <div class="col-sm-3">
                            <div class="form-group form-float">
                                <div class="form-line {{ $errors->has('email') ? 'focused error' : '' }}">
                                    <label>Email*</label>
                                    <input type="email" name="email" class="form-control" required value="{{  $errors->has('email') ? old('email') : $user->email   }} " maxlength="255" >                        
                                </div>
                                @if ($errors->has('email'))
